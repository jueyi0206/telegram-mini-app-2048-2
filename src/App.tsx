...
export default function App() {
  ...
  const move = useCallback((direction: Direction) => {
    setGameState((prevState) => {
      ...
      // Check Game Over AFTER spawning the new tile
      // We need to check if the grid is full AND no moves are possible
      const isGridFull = tiles.length === GRID_SIZE * GRID_SIZE;
      const gameOver = isGridFull && !canMove(tiles);
  ...
  const canMove = (currentTiles: Tile[]) => {
    // 1. If there are empty cells, we can move
    if (currentTiles.length < GRID_SIZE * GRID_SIZE) return true;

    // 2. Check for possible merges between neighbors
    for (const tile of currentTiles) {
      // Check Right and Down neighbors (sufficient to find all pairs)
      for (const dir of [[1, 0], [0, 1]]) {
        const targetX = tile.x + dir[0];
        const targetY = tile.y + dir[1];

        // Boundary check
        if (targetX >= 0 && targetX < GRID_SIZE && targetY >= 0 && targetY < GRID_SIZE) {
          const neighbor = currentTiles.find(t => t.x === targetX && t.y === targetY);
          // If neighbor exists and values match, a move (merge) is possible
  ...
  const shareScore = () => {
    ...
    const shareUrl = "https://t.me/share/url?url=" + encodeURIComponent(GAME_URL) + "&text=" + encodeURIComponent(text);
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 grid grid-cols-3 gap-2 w-full max-w-[200px]">
        <div />
        <button 
          className="bg-neutral-800 p-4 rounded-xl active:bg-neutral-700 active:scale-95 transition-all flex justify-center items-center"
          onClick={() => move('UP')}
          aria-label="Up"
        >
